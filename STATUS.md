...
**Next session (planned order).** (1) **FU-20** (ship `templates/.llms/commands/` for Devmate + the loading check) — *maybe*. (2) then the **benchmark replay harness + routing v0** (tests_action shipped 2026-07-06 — the oracle is now real; **FU-44** scopes `tests_pass` to the acceptance suite and rides the harness). Proposal B (the `i2c refine` loop + the `/refine` bot) was removed unused on 2026-08-07 (see CHANGELOG); the fix/bugfix consolidation that would have built on it is unblocked only if that loop is first revived.
