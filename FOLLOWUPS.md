...
1. **FU-35** — Anthropic `cache_control` markers on the stable prompt prefix. ~10 LOC in `run_iteration.py`'s claude path; shippable now (not gated on 3.C); reliability-neutral; OpenAI/Gemini already benefit from the FU-24 region order. Measure savings via `tokens_cached` (FU-33) before deciding 3.C is still worth it.
2. **FU-32 Δ5** — PLAN precondition check that escalates if ARCH lacks Required sections. Deferred until template stabilizes (≥1 more CC ARCH authored under the Pattern A/B v2 template). ~15 lines of doc once we know what works.
3. **Phase 3.C** — multi-iteration loop. **Re-evaluate after FU-35 caching data** (caching likely captures most of the token-savings motivation; 3.C's remaining justification is cross-step reasoning continuity + fewer spin-ups, weighed against the e2e-vintage multi-iteration reliability cost). Wrap `run_iteration.py` once we have real data on what single-iteration shape misses. The `multi_step_only` marker mechanism is forward-compatible; runner just needs to start passing `--step-budget > 1`.
...
| FU-35 | Runner doesn't emit Anthropic `cache_control` markers around stable prompt prefix | open | Wrapping WORKER CONTRACT and TOOL RULES regions with Anthropic's ephemeral `cache_control` blocks lets us hit the prompt cache on iter 2+ within a phase. **Not gated on Phase 3.C** (un-gated 2026-06-21): consecutive single-iteration invocations of the same phase already share a long stable prefix (WORKER CONTRACT + TOOL RULES + most of PROJECT CONTEXT; only the step pointer and devlog tail change), so iter 2+ hits cache *today* as long as the loop re-invokes within the ~5-min ephemeral TTL. The FU-24 region order (stable-first, volatile-last) is already prefix-cache-optimal. Reliability-neutral: cached tokens are byte-identical input, so output quality is unchanged; the only miss-case cost is a one-time ~25% cache-write premium. Other providers (OpenAI, Gemini) auto-detect identical prefixes without explicit markers, so the current prompt shape already benefits them; this item is Anthropic-specific. | Shippable now, independent of Phase 3.C. ~10 LOC in `run_iteration.py`'s claude invocation path. Verify cache hits via `cache_read_tokens` (already logged as `tokens_cached` per FU-33) on the second iteration of a phase. Measured savings then inform whether 3.C's additional token amortization is still worth its multi-iteration reliability cost. |
