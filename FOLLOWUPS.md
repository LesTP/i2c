...
| FU-30 | `state_machine.py` has no "all phases done" terminus; `EXIT` only dispatches when `blocked=true` | open (pilot-confirmed) | The dispatch matrix in `tools/state_machine.py::decide` only returns `ACTION: EXIT` when `blocked == True`. There is no branch that inspects `phases.json` to detect "every phase complete; no more work to do." With `blocked=false, state=close` the machine dispatches `CLOSE` again (worker would redundantly re-close the last completed phase); with `blocked=false, state=plan, phase=N` where N has no record, it dispatches `PLAN` and the worker escalates per `plan.md` ("no record for this phase number"). **The gate (`blocked=true`) is therefore doing double duty:** "waiting on human audit between phases" AND "all phases done, project terminal." Conflating those two states means the operator cannot signal "done forever" distinctly from "waiting on audit." **CC pilot completion (2026-06-07):** assistant suggested "clear the gate to dispatch EXIT (pilot done)" after Phase 3 CLOSE. Cleared `blocked=false`; `state_machine` returned `ACTION: CLOSE NEXT: plan` (would have re-run close worker on already-closed phase 3). Restored `blocked=true` immediately; correct EXIT dispatch resumed. | Three viable fix models — pick one: **(a)** Document the convention that `blocked=true` IS the terminus (no code change; cheapest; conflates audit-gate with done-forever). **(b)** Extend `state_machine.py::decide`: if every `phases.json` record has `status=complete` AND `project.json.phase` has no successor pending record, return `ACTION: EXIT NEXT: close` regardless of `blocked`. ~5 LOC; smallest behavioral change; removes the gotcha without new schema. **(c)** Add terminal `state=done` to `project.schema.json`'s state enum; `state_machine.py` returns `EXIT` when `state=done`; `close.md` instructs the CLOSE worker to set `state=done` after the final phase. Most explicit but biggest change. **Recommended: (b)** for the immediate fix, with (a)'s documentation update as a companion. |
