...
| FU-2 | No subcommand to append a new step mid-phase | partially closed | `state.py append-record steps.json '{...}'` exists as of the prose-layer build (covers step authoring at PLAN time and in principle mid-phase too). The original framing of this gap was specifically about EXECUTE-time step creation though, which `instructions/execute.md` still funnels through devlog `Deferred:` flags so PLAN owns step authoring. If a real workflow demands runtime step append from EXECUTE, lift the restriction in execute.md prose — the CLI now supports it. | Phase 2 pilot shows execute-time step append is wanted; loosen the prose. |
...
| FU-14 | No read-side query helper in `state.py` (e.g., `state.py query devlog.jsonl --where 'contracts != []'`) | accepted (deferred) | The assembler now exists (Phase 1.3) and exposes pre-formatted views: `--section devlog --phase N` gives a bulleted phase tail, `--section status` gives an orientation snapshot. Ad-hoc queries (e.g., "devlog rows with non-empty contracts") still fall back to `jq`. Per ARCH §2 / §10, the assembler intentionally does not absorb general-purpose queries — `jq` remains the answer for one-offs. Revisit only if Phase 2 pilot shows the same `jq` query repeatedly. | Phase 2 pilot reveals a repeated query pattern worth absorbing into `--section`. |
| FU-13 | No `state.py` op to flip a record's status from `pending` to `in_progress` | closed | Resolved by `state.py update-record` (added when authoring `instructions/review.md` + `close.md`). Now atomic: `state.py update-record phases.json --match id=11 status=in_progress`. Same op covers any field update on any matched record across all three array files. |

## Tooling — assembler (Phase 1.3, not yet started)

| ID | Title | Status | Context | Trigger to address |
|----|-------|--------|---------|--------------------|
| FU-5 | Conditional-section detection needs `dependencies` lookup | closed | Resolved by Phase 1.3. `tools/assemble_context.py` implements the marker extractor + evaluator registry per ARCH §7. `requires=dependencies_nonempty` evaluator reads `phases.json[id==current_phase].dependencies`; `autonomous_only` / `supervised_only` evaluators key off `--mode`. Verified against fixture (phase 2 leaf strips dep-probe; phase 4 non-leaf keeps it). |
| FU-6 | Smoke test doesn't exercise the non-leaf Phase 4 path | closed | Resolved by Phase 1.3. `tests/test_assemble_context.py` covers both leaf (phase 2) and non-leaf (phase 4) plan paths, asserting Dependency Probe section is stripped vs kept correctly. `examples/smoke_test.py` exercises an assembler `--section status` call against the fixture at the end. |
...
| FU-13 | `update-record FILE --match KEY=VALUE field=value ...` added when authoring `instructions/review.md` + `close.md` surfaced the need to close open decisions and (in principle) flip phase status mid-flight. Generic: matches one record by a single key=value (errors on no-match or multi-match), updates one-or-more fields, validates the whole array, atomic write. Sibling to `append-record` in pattern. |
| FU-5 | Phase 1.3 `tools/assemble_context.py` implements the conditional-section marker mechanism per ARCH §7 (evaluator registry, `requires=dependencies_nonempty`, `autonomous_only`, `supervised_only`). Markers in `instructions/plan.md` and `instructions/close.md` now strip deterministically. |
