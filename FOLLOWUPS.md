...
| FU-38 | Add backends beyond claude/codex — Gemini and OpenRouter | open (roadmap; design captured) | The operator keeps hitting per-backend rate/usage limits and wants a third backend; OpenRouter (already used in Diplomat experiments) gives one adapter over many models. Key design fork: claude/codex are *agentic CLIs* (they run tools themselves); Gemini-API/OpenRouter are *raw APIs* needing an agentic harness (use Gemini's agentic CLI if viable; for OpenRouter, point an existing harness at it or grow a minimal i2c agentic loop). Caching is already a per-backend capability (FU-35 / D-pkg-5). Also worth validating: i2c's deterministic governance may let weaker, non-frontier models drive real builds (agentic reliability, not governance comprehension, is the risk). Full design in `DESIGN_packaging_v1.md` §6.1/§6.2, D-pkg-12/D-pkg-13. | Sequence with the backend-protocol work (packaging Phase 3); pulled forward whenever the operator decides to add Gemini. |
