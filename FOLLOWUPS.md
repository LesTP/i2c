...
| ID | Title | Status | Context | Trigger to address |
|----|-------|--------|---------|--------------------|
| FU-12 | Multi-line JSON / `$`-laden state.py payloads on PowerShell | open (code-side fixed; doc-side pending) | The examples in `instructions/execute.md` use `'{ "key": value }'` with embedded newlines and `$`-tokens. PowerShell quoting rules differ — backtick-vs-backslash, `$`-interpolation. **Code fix shipped** as FU-21 (`--from-file` flag family on all four payload subcommands). Verified safe via the `$defs`/`$refs` gotcha-string round-trip test. **Remaining:** adapter `CLAUDE.md` / `CODEX.md` Tool Rules in both i2c and CC do not yet point operators / workers at `--from-file` as the recommended path for multi-line or `$`-laden payloads. | Update Tool Rules in `i2c/CLAUDE.md`, `i2c/CODEX.md`, `clankercourts/CLAUDE.md`, `clankercourts/CODEX.md` to add a one-paragraph recommendation: "For multi-line JSON or any payload containing `$` characters, write the payload to a temp file and pass `--from-file <path>`." Then close. |
...
| FU-28 | Meta laptop sandbox prevents `claude -p` subprocess autonomy; consumer projects must invoke the loop from a server | open (workaround in production; doc still pending) | Running `python3 ../i2c/tools/run_iteration.py --backend claude ...` from a Meta-issued Windows laptop hangs indefinitely. The wrapper chain `claude` → `dotslash` → `fast_mux` → bun-bundled `claude.exe` doesn't terminate cleanly under sandboxed non-interactive stdin. Even a trivial standalone `"say hi" \| claude -p` hangs >5 minutes with no output. The Meta sandbox restricts autonomy (subprocess pipe handling, child-process exit semantics, or both) such that `claude -p` is non-functional for autonomous-loop purposes. **CC autonomous-loop iteration 1 attempt (2026-06-06):** confirmed in production; runner timed out after 25min with no claude output, no `iteration_NNN.txt` written, no exit signal. `fast_mux.exe` was still alive holding stdout open after the inner Bun crashed. **Workaround applied (now standard practice):** invoke the loop from `pirozhok` (Raspberry Pi server) where the consumer project is mounted via Samba on the same disk as the laptop's mapped drive. Pattern: `ssh pirozhok "incus exec claude-code -- su - claude -c 'cd /home/claude/workspace/<consumer> && python3 ../i2c/tools/run_iteration.py ...'"`. CC Phases 2–4 all ran cleanly under this pattern. | Add an explicit "Meta-laptop / non-Linux note" subsection to the Invocation guidance below: laptop = supervised-only; server = autonomous; Samba-mounted shared disk lets the same `.state/` see both modes without sync friction. Runner code doesn't need a fix — only the operational guidance is incomplete. |
