...
| FU-13 | No `state.py` op to flip a record's status from `pending` to `in_progress` | open | `complete` jumps straight to `complete`. `set` doesn't work on array files (FU-3). Today, PLAN can only create new phase records (status=`in_progress` from the start) or leave existing `pending` records as-is. The same gap exists for steps (where it's documented in execute.md). Workaround: rely on `project.json.phase` to identify the active phase regardless of `phases.json` status. | Phase 2 pilot reveals a real workflow where `pending → in_progress` needs to be atomic and observable to downstream tools (waymark sidebar, codexbot `/status`). Then add `state.py mark FILE --phase N [--step M] --status STATUS`. |
