...
| FU-32 | PLAN action not yet autonomous-capable; needs five framework deltas + ARCH-file discipline | **open** (planned, not started) | After CC Phase 4 EXECUTE shipped supervised (commit `97e9ea4`), the meta-question surfaced: i2c's autonomous loop runs EXECUTE/REVIEW/CLOSE cleanly, but PLAN's step-breakdown step still requires human authoring because ARCH files aren't constrained enough to drive mechanical step decomposition. e2e solves this via a two-step workflow (pre-arch design separately, autonomous batch implementation); i2c lacks the ARCH-authoring discipline and the safety-net escalation triggers that make autonomous PLAN safe. Five deltas identified: (Δ1) optional `phase: integer` field on `decisions.schema.json`; (Δ2) explicit escalation-triggers enumeration in `instructions/plan.md` step 2 (source-vs-ARCH drift, multi-regime scope, cross-module breakage at plan time, step-shape ambiguity, missing precondition); (Δ3) `assemble_context.py --section decisions --phase N` for batch phase-boundary review; (Δ4) port `e2e/ref/SPEC_architecture.md` + `e2e/ref/GUIDE_architecture.md` into `i2c/ref/` with two new required sections in the per-module template (`## Phasing in this pilot` and `## Escalation triggers`) plus two recommended sections (`## Testing Strategy`, `## Inputs the Module Does Not Handle`); (Δ5) PLAN precondition check that escalates to `audit_escalation` if the assembled Module Contract lacks the required sections. Full plan + sequencing options (CC-first vs i2c-first) + cold-start summary in `NEXT_STEPS_autonomous_plan.md`. | Pick Path A (author CC Phase-5+ ARCH files first, then codify deltas from lessons) or Path B (land Δ1-Δ4 in i2c first, then CC authors against the templates). Either way: ~one session for Δ1-Δ4 + one session per CC module ARCH; Δ5 lands after first real ARCH validates the templates. Goal: cut per-phase human touch points from ~15 (Phase 4) to ~3-4 (ARCH-design session + phase-boundary review + clearance). |
