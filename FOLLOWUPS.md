...
| FU-14 | No read-side query helper in `state.py` (e.g., `state.py query devlog.jsonl --where 'contracts != []'`) | open | Contract scan in `instructions/close.md` uses raw `jq` queries. Reads don't need state.py's atomicity guarantee, and the eventual home for pre-formatted, schema-aware queries is `assemble_context.py` (Phase 1.3). | Phase 1.3 fails to absorb the read-helper need cleanly, OR Phase 2 pilot shows workers/operators frequently bouncing through `jq` for the same queries. Then add a small `state.py query` op or expose canned queries via the assembler. |
...
| ID | Title | Status | Context | Trigger to address |
|----|-------|--------|---------|--------------------|
| FU-5 | Conditional-section detection needs `dependencies` lookup | spec locked | DESIGN D19 / D-prose-1: assembler includes dep-probe section in plan + integration-check section in close when the current phase's `dependencies` array is non-empty. Fixture already demonstrates this (Phase 4 `orchestrator` depends on `event_store`). Per `ARCH_assembler.md` §7 (D-arch-3), the resolution is the heading-metadata-marker mechanism: instruction files carry `<!-- assembler:requires=dependencies_nonempty -->` after the relevant heading; the assembler evaluates the key against `phases.json[id=$PHASE].dependencies` and strips deterministically. Markers are already inserted in `plan.md` (Pre-plan Dependency Probe) and `close.md` (Pre-close Integration Check). | Phase 1.3 (assembler build) — implements the marker extractor + dependency-evaluator. |
