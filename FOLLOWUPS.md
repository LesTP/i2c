...
| FU-38 | Add backends beyond claude/codex — Gemini and OpenRouter | open (roadmap; **provider layer done in `toolkit.llm_client`**) | **Provider/transport layer is done and proven.** `toolkit.llm_client` already wraps Anthropic / OpenAI / **Google (Gemini)** / **OpenRouter** with multi-provider rotation, rate-limit tracking, a budget tracker, and the OpenRouter reasoning-content fallback; battle-tested in Diplomat (incl. OpenRouter R1/Qwen runs). But it is a **raw-completion** API (`LLMProvider.call(messages) -> text`), so it closes the transport half, **not** i2c's agentic half: i2c worker backends are *agentic CLIs* (`run_iteration` shells `claude -p` / `codex exec`, which run tools, write `.state/`, commit, and emit the 2-line signal). **Residual:** (1) **Gemini** — add as an agentic-CLI subprocess backend mirroring `invoke_claude` / `invoke_codex` (cheapest path; no harness, no `llm_client` needed). (2) **OpenRouter / raw-API models** — build a minimal **agentic harness** over `llm_client.call()` (tool-call loop: execute edits/shell/state writes, feed results back, iterate to the 2-line signal) satisfying the `invoke_*` seam; this is where "can weaker, non-frontier models drive real builds" gets tested (agentic reliability, not governance comprehension, is the risk). (3) **toolkit dep = OpenRouter-only; ship as an optional extra.** Gemini (path 1) needs no new dep; the OpenRouter harness (path 2) is the only thing pulling `toolkit.llm_client`, so quarantine it behind an optional extra — `pip install i2c[openrouter]` — mirroring `i2c[telegram]` (D-pkg-10: thin optional extras). Caveat: `toolkit` isn't on PyPI (it's `LesTP/toolkit` over git), so the extra is **internal-only** until toolkit is publishable or the minimal provider code is vendored — the live half of D-pkg-12. Caching is already a per-backend capability (FU-35 / D-pkg-5). Full design in `DESIGN_packaging_v1.md` §6.1/§6.2, D-pkg-12/D-pkg-13. | Sequence with the backend-protocol work; pulled forward whenever the operator wants the Gemini CLI path or a weaker-model OpenRouter build (the harness path). |
